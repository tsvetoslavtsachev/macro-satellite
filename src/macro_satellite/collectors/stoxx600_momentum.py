"""Парсер за stoxx600-momentumrank/data.json (~600 stocks).

Същата схема като SP500 momentum + 3 допълнителни полета: country, exchange, currency.
"""
...
def parse(raw: bytes, snapshot_date: date | None = None,
          source: str = "stoxx600_momentum") -> pd.DataFrame:
    items = json.loads(raw)
    if not isinstance(items, list):
        raise ValueError("stoxx600_momentum: expected JSON array")
    ...
    for it in items:
        ...
        row["market_cap"] = None  # not in STOXX600 schema
